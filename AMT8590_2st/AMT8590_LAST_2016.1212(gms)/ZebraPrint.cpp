...
bool CZebraPrint::OnPrintStatus(CString strCheckData, int nPort)
{
	...
	{
		strBody.Replace("\2", "");		// 중간에 있는 STX 제거
		strBody.Replace("\3", ",");		// 중간에 있는 ETX를 ,로 변경
	}
	else
	{
		strBody = strBody + ",";
...
void CZebraPrint::OnPrintAnswerMode(int nMethod, int nMode, int nPort)//2014.1104 ysj
{
	...
	{
		if (nMode == 0)
		strPrint = "^XA^SXK,D,Y,Y,192.168.1.16,9100^XZ";	 // print 응답 확인 
		else 
		strPrint = "^XA^SXK,D,N,N,192.168.1.16,9100^XZ";	 // print 응답 확인 
	}
	else if(nPort == CLS_BCR_PRINTER2)
	{
		if (nMode == 0)
			strPrint = "^XA^SXK,D,Y,Y,192.168.1.16,9101^XZ";	 // print 응답 확인 
		else 
			strPrint = "^XA^SXK,D,N,N,192.168.1.16,9101^XZ";	 // print 응답 확인 
	} 
	else if(nPort == CLS_BCR_PRINTER3)
	{
		if (nMode == 0)
			strPrint = "^XA^SXK,D,Y,Y,192.168.1.16,9102^XZ";	 // print 응답 확인 
		else 
			strPrint = "^XA^SXK,D,N,N,192.168.1.16,9102^XZ";	 // print 응답 확인 
	} 
	else if(nPort == CLS_BCR_PRINTER4)
	{
		if (nMode == 0)
			strPrint = "^XA^SXK,D,Y,Y,192.168.1.16,9103^XZ";	 // print 응답 확인 
		else 
			strPrint = "^XA^SXK,D,N,N,192.168.1.16,9103^XZ";	 // print 응답 확인 
	}
