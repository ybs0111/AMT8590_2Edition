...
LRESULT CButtonST::DefWindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
		GetParent()->PostMessage(WMU_CHILD_LBUTTON_DOWN, 0, (LPARAM)this);
		break;
	case WM_LBUTTONUP:
		GetParent()->PostMessage(WMU_CHILD_LBUTTON_UP, 0, (LPARAM)this);
		break;
	}
...
DWORD CButtonST::OnDrawBorder_OLD(CDC *pDC, LPCRECT pRect)
{
	if (m_bIsPressed)
		pDC->Draw3dRect(pRect, ::GetSysColor(COLOR_BTNSHADOW), ::GetSysColor(COLOR_BTNHILIGHT));
	else
