...
void CDigit::OnTimer(UINT nIDEvent) 
{
...
void CDigit::MakeString()
{
	...
	{
	case DS_INT:
		m_strData.Format("%d", m_nVal);
		diff = m_nPrec - m_strData.GetLength();
		for(i=0; i<diff; i++) m_strData = " " + m_strData;
		break;
	case DS_FLOAT:
		m_strData.Format("%4.1f", m_fVal);
		diff = m_nPrec - m_strData.GetLength();
		for(i=0; i<diff; i++) m_strData = " " + m_strData;
		break;
	case DS_TIMER:
		m_strData.Format("%.2d:%.2d:%.2d", m_nT[0], m_nT[1], m_nT[2]);
		break;
	case DS_CHAR:
		m_strData = m_strChar;
		diff = m_nPrec - m_strData.GetLength();
		...
		break;
	case DS_FLOAT2:
		m_strData.Format("%4.2f", m_fVal);
		diff = m_nPrec - m_strData.GetLength();
		for(i=0; i<diff; i++) m_strData = " " + m_strData;
		break;
	case DS_FLOAT3:
		m_strData.Format("%4.3f", m_fVal);
		diff = m_nPrec - m_strData.GetLength();
		for(i=0; i<diff; i++) m_strData = " " + m_strData;
		break;
	case DS_CLOCK:
		...
		if(!bSame) m_strData.Format("%.2d:%.2d:%.2d", m_nT[0], m_nT[1], m_nT[2]);
		else m_strData.Format("%.2d:%.2dx%.2d", m_nT[0], m_nT[1], m_nT[2]);
		break;
...
{
	if(m_nStyle == DS_FLOAT) m_strData.Format("Err.%.1d", m_nErrNum);
	else if(m_nStyle == DS_INT) m_strData.Format("Err%.1d", m_nErrNum);
	else if(m_nStyle == DS_FLOAT2) m_strData.Format("Err%.2d", m_nErrNum);
	else if(m_nStyle == DS_FLOAT3) m_strData.Format("Err%.3d", m_nErrNum);
}
